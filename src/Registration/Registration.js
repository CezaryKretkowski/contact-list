...
export default class Registration extends Component {
    ...
    handleSubmit =(event)=>{
        ...
        if(this.state.categoryId===1){
            ...
            this.setState({ error:"Passwords doesn not match!"});
        }
    ...
    return (
      <div className='Registration'>
        ...
        <TextField style={{marginLeft:50,marginTop:20}} required label="Password" variant="filled" type='Password' 
         inputProps={{ pattern:"(?=.*)(?=.*[a-z])(?=.*[A-Z]).{8,}"}} 
        onChange={(e)=>this.setState({ password: e.target.value})}/>
        <TextField style={{marginLeft:50,marginTop:20}} required label="Confirm Password" variant="filled"  type='Password' onChange={(e)=>this.setState({ cPassword : e.target.value})}/>   
        ...
        <Button style={{marginLeft:'10%',marginTop:'30px',marginBottom:30}} type="submit" variant="contained" onClick={this.submitHandle}>Sing in</Button>
        </form>
      </div>
    )
  }
